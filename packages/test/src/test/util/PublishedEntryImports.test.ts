 ...
 * `"./packages/*"` → `"packages"`.
 *
 * This is duplicated DERIVATION CODE, not a duplicated list. The tooling
 * keeps its own list under `scripts/`, pinned against this same manifest field
 * by `scripts/workspaceSource.test.ts`; this file cannot import it, for the
 * reason spelled out on the `describe` below — `packages/test` is a `composite`
 * project rooted at `./src`, so importing from `scripts/` would put those files
 * in its program and break `build-types`. Re-deriving still beats copying the list: a group
